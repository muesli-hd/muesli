...
class UnloggedTests(BaseTests, functionalTests.PopulatedTests):
    ...
    def test_exam_enter_points_tuts(self):
        res = self.testapp.get('/exam/enter_points/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                               status=403)

    def test_exam_enter_points_raw_tuts(self):
        res = self.testapp.get('/exam/enter_points_raw/%s/%s,%s?students=%s' % (
        self.exam.id, self.tutorial.id, self.tutorial2.id, self.user.id), status=403)

    def test_exam_statistics(self):
        res = self.testapp.get('/exam/statistics/%s/' % self.exam.id, status=403)
        res = self.testapp.get('/exam/statistics/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                               status=403)

    def test_exam_histogram_for_exercise(self):
        res = self.testapp.get('/exam/histogram_for_exercise/%s/' % self.exercise.id, status=403)
        res = self.testapp.get(
            '/exam/histogram_for_exercise/%s/%s,%s' % (self.exercise.id, self.tutorial.id, self.tutorial2.id),
            status=403)

    def test_exam_histogram_for_exam(self):
        res = self.testapp.get('/exam/histogram_for_exam/%s/' % self.exam.id, status=403)
        res = self.testapp.get(
            '/exam/histogram_for_exam/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id), status=403)

    def test_exam_export(self):
        res = self.testapp.get('/exam/export/%s/' % self.exam.id, status=403)

    def test_exam_export_tuts(self):
        res = self.testapp.get('/exam/export/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                               status=403)
    ...
    def test_exam_admission_tuts(self):
        res = self.testapp.get('/exam/admission/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                               status=403)

    def test_exam_ajax_save_points(self):
        res = self.testapp.get('/exam/ajax_save_points/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                               status=403)

    def test_exam_ajax_get_points(self):
        res = self.testapp.get('/exam/ajax_get_points/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                               status=403)

    def test_exam_correlation(self):
        res = self.testapp.get('/exam/correlation?source1=exam_%s&source2=exam_%s' % (self.exam.id, self.exam.id),
                               status=403)
        res = self.testapp.get('/exam/correlation?source1=lecture_%s&source2=exam_%s' % (self.lecture.id, self.exam.id),
                               status=403)
        res = self.testapp.get('/exam/correlation?source1=exam_%s&source2=lecture_%s' % (self.exam.id, self.lecture.id),
                               status=403)
        res = self.testapp.get(
            '/exam/correlation?source1=lecture_%s&source2=lecture_%s' % (self.lecture.id, self.lecture.id), status=403)
...
class TutorLoggedInTests(UserLoggedInTests):
    ...
    def test_exam_enter_points_raw_tuts(self):
        res = self.testapp.get('/exam/enter_points_raw/%s/%s,%s?students=%s' % (
        self.exam.id, self.tutorial.id, self.tutorial2.id, self.user.id), status=200)
    ...
    def test_exam_histogram_for_exam(self):
        ...
        res = self.testapp.get(
            '/exam/histogram_for_exam/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id), status=200)
    ...
    def test_exam_ajax_save_points(self):
        ...
        res = self.testapp.post('/exam/ajax_save_points/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                                post, status=200)
    ...
    def test_exam_ajax_get_points(self):
        post = {'student_id': self.user.id}
        res = self.testapp.post('/exam/ajax_get_points/%s/%s,%s' % (self.exam.id, self.tutorial.id, self.tutorial2.id),
                                post, status=200)
        self.assertTrue(res.status.startswith('200'))
        data = res.json
    ...
    def test_exam_correlation(self):
        ...
        res = self.testapp.get('/exam/correlation?source1=exam_%s&source2=lecture_%s' % (self.exam.id, self.lecture.id),
                               status=200)
        res = self.testapp.get(
            '/exam/correlation?source1=lecture_%s&source2=lecture_%s' % (self.lecture.id, self.lecture.id), status=200)

        # res = self.testapp.get('/exam/correlation?source1=exam_%s&source2=exam_%s' % (self.exam.id, self.exam.id), status=403)
        res = self.testapp.get(
            '/exam/correlation?source1=lecture_%s&source2=exam_%s' % (self.lecture2.id, self.exam.id), status=403)
        res = self.testapp.get(
            '/exam/correlation?source1=exam_%s&source2=lecture_%s' % (self.exam.id, self.lecture2.id), status=403)
        res = self.testapp.get(
            '/exam/correlation?source1=lecture_%s&source2=lecture_%s' % (self.lecture.id, self.lecture2.id), status=403)
        res = self.testapp.get(
            '/exam/correlation?source1=lecture_%s&source2=lecture_%s' % (self.lecture2.id, self.lecture.id), status=403)
