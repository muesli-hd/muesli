...
class CSRFSecureForm(Form):
    def __init__(self, formfields, request, send="Ändern", chained_validators=[]):
        token_field = HiddenField('csrf_token', value=request.session.get_csrf_token(),
...
class LectureAdd(ObjectForm):
    def __init__(self, request):
        ...
        if request.permissionInfo.has_permission('change_assistant'):
            assistants = request.db.query(models.User).filter(models.User.is_assistant == 1).order_by(
...
class UserRegister(ObjectForm):
    def __init__(self, request):
        formfields = [
            ...
            FormField('matrikel',
                      label='Matrikelnummer', size=10,
                      comment='Falls noch keine Matrikelnummer bekannt ist bitte 00000 eintragen. Die Matrikelnummer muss dann baldmöglichst unter „Angaben ergänzen“ richtig gestellt werden!',
                      validator=validators.Number,
                      # value=user.matrikel,
                      required=True
                      ),
