...
class Allocation:
    ...
    def doAllocation(self):
        ...
        for time in times:
            time_tutorials = []
            tuts = self.session.query(models.Tutorial).filter(models.Tutorial.lecture_id == self.lecture.id).filter(
        ...
        return {'error_value': 0,
                ...
                'global_happiness': 100.0 * global_happiness / (len(students) - len(students_without_group)) if len(
                    students) != len(students_without_group) else 0
                }
